export const FoundersNote = () => {
  return (
    <section className="relative content-center items-center box-border caret-transparent gap-x-11 flex flex-col shrink-0 h-min justify-center gap-y-11 w-full overflow-hidden px-[18px] py-20 md:px-10 md:py-[100px]">
      <div className="relative content-center items-center box-border caret-transparent gap-x-6 flex flex-col shrink-0 flex-wrap h-min justify-start max-w-[1200px] gap-y-6 w-full z-[2]">
        <div className="relative content-center items-center box-border caret-transparent gap-x-6 flex flex-col shrink-0 h-min justify-center gap-y-6 w-full z-[3] pr-0 md:pr-5">
          <div className="relative box-border caret-transparent shrink-0 z-[4]">
            <div className="relative content-center items-center box-border caret-transparent gap-x-2.5 flex h-min justify-start gap-y-2.5 w-min rounded-[60px]">
              <div className="relative content-center items-center backdrop-blur-[68px] bg-black box-border caret-transparent gap-x-2.5 flex shrink-0 h-min justify-start gap-y-2.5 w-min z-[2] overflow-hidden px-4 py-2 rounded-[26px]">
                <div className="relative content-center items-center box-border caret-transparent gap-x-1 flex shrink-0 h-min justify-start gap-y-1 w-min">
                  <div className="relative box-border caret-transparent flex flex-col shrink-0 justify-start text-nowrap">
                    <p className="text-base font-semibold box-border caret-transparent tracking-[-0.32px] leading-[25.6px] text-left text-nowrap font-inter">
                      <span className="bg-clip-text bg-[linear-gradient(105deg,rgb(138,165,255)_22.3694%,rgb(133,77,255)_180%)] box-border caret-transparent inline-block text-nowrap">
                      MENSAGEM DO FUNDADOR
                      </span>
                    </p>
                  </div>
                </div>
              </div>
              <div className="absolute bg-[linear-gradient(105deg,rgb(41,52,255)_-8%,rgba(36,65,212,0)_50%)] box-border caret-transparent shrink-0 z-[1] overflow-hidden rounded-[22px] -left-0.5 -right-px -inset-y-px"></div>
            </div>
          </div>
        </div>
        <div className="relative content-center items-center box-border caret-transparent gap-x-6 flex flex-col shrink-0 flex-wrap h-min justify-center max-w-screen-sm gap-y-6 w-full z-[2]">
          <div className="relative box-border caret-transparent flex flex-col shrink-0 justify-start break-words w-full">
            <h3 className="text-white text-2xl box-border caret-transparent tracking-[-0.72px] leading-[33.6px] break-words text-center font-inter md:text-[32px] md:tracking-[-0.96px] md:leading-[44.8px]">
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                “
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                We
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                gather
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                your
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                site
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                data.
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]"></span>
              <span className="text-indigo-400 text-2xl box-border caret-transparent tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:tracking-[-0.96px] md:leading-[44.8px]">
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  We
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  know
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  your
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  target
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  audience
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  &amp;
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  how
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  your
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  brand
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  can
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  standout
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  from
                </span>
                <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                  crowd.
                </span>
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]"></span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                Best
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                part
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                is
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                we
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                also
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                help
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                you
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                with
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                Solutions
              </span>
              <span className="text-2xl box-border caret-transparent inline-block tracking-[-0.72px] leading-[33.6px] break-words md:text-[32px] md:blur-0 md:tracking-[-0.96px] md:leading-[44.8px]">
                „
              </span>
            </h3>
          </div>
          <div className="relative content-center items-center box-border caret-transparent gap-x-4 flex flex-col shrink-0 flex-wrap h-min justify-center gap-y-4 w-full md:flex-row">
            <div className="relative content-center items-center box-border caret-transparent gap-x-6 flex shrink-0 h-min justify-center gap-y-6 w-min">
              <div className="relative aspect-square shadow-[rgb(255,255,255)_0px_0px_0px_2px] box-border caret-transparent shrink-0 w-7 z-[1] overflow-hidden rounded-[1000px]">
                <div className="absolute box-border caret-transparent rounded-[1000px] inset-0">
                  <img
                    sizes="28px"
                    src="https://c.animaapp.com/mkvwf206SqHHjG/assets/29.jpg"
                    alt="user pic"
                    className="aspect-[auto_4000_/_4000] box-border caret-transparent h-full object-cover w-full rounded-[1000px]"
                  />
                </div>
              </div>
            </div>
            <div className="relative content-center items-center box-border caret-transparent gap-x-1 flex shrink-0 h-min justify-center gap-y-1 w-min overflow-hidden">
              <div className="relative box-border caret-transparent flex flex-col shrink-0 justify-start text-nowrap">
                <p className="text-violet-100/70 text-base box-border caret-transparent tracking-[-0.32px] leading-[25.6px] text-left text-nowrap font-inter">
                  Fundador & Fisioterapeuta
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="absolute bg-[radial-gradient(63.6719%_63.6719%,rgb(138,165,255)_0%,rgb(0,0,0)_100%)] box-border caret-transparent shrink-0 h-1 left-[calc(50%_-_640px)] opacity-[0.14] w-[1280px] z-[1] top-0 md:w-screen"></div>
      <div className="absolute bg-[radial-gradient(50%_50%,rgb(18,20,38)_0%,rgb(0,0,0)_100%)] bottom-[-249px] box-border caret-transparent shrink-0 h-[378px] left-[calc(50%_-_300px)] rotate-[-12.999997801129437deg] w-[600px] z-[1] rounded-[10px] md:h-[499px] md:left-[calc(50%_-_396.5px)] md:w-[793px]"></div>
    </section>
  );
};
